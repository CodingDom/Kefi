...
export class MapComponent implements AfterViewInit {
  ...
  createMarkers() {
    ...
    for (let i = 0; i < this.properties.length; i++) {
      ...
      if (this.popUp) {
        marker.addTo(this.map).bindPopup(`<div style="width:150px">
              <div style="height:66px;"></div>
        <img src="${info.images[0]}" width="100%" height="80px" style="position:absolute;top:0;left:0;object-fit:cover;"/>
        ...
        <p>${info.title}</p>			
        <div class="text-center"><a href="/property/${info.airbnb_property_id}" class="btn-accent py-2 px-4" style="color:white;">View</a></div>
        </div>`);
      } else {
