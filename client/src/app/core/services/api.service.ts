...
export class ApiService {
  ...
  public getLocations(location: string) {
    ...
    return fetch(`${this.API_BASE_URL}/locations?location=${location}`).then(
      (resp) => resp.json()
    );
  }
  ...
  public getProperties(
    cityId: number,
    options?: PropertyListQueryOptions
    ...
    return fetch(
      `${this.API_BASE_URL}/properties?cityId=${cityId}${extras}`
    ).then((resp) => resp.json());
  }

  public getAirdnaPropertyDetails(
    propertyId: string
  ): Promise<AirdnaPropertyDetails> {
    return fetch(`${this.API_BASE_URL}/properties/${propertyId}`).then((resp) =>
      resp.json()
    );
  }

  public getVrboPropertyDetails(
    propertyId: string
  ): Promise<VrboPropertyDetails> {
    return fetch(`${this.API_BASE_URL}/vrbo/${propertyId}`).then((resp) =>
      resp.json()
    );
  }

  public getTravelNews(): Promise<TravelNews[]> {
    return fetch(`${this.API_BASE_URL}/travel-news`).then((resp) =>
      resp.json()
    );
  }
